...
final class Ultimate_PortfolioUITests: XCTestCase {
    ...
    func testNoIssuesAtStart() {
        XCTAssertEqual(app.cells.count, 0, "There should be 0 list row initially.")
    }
    ...
    func testEditingIssueTitleUpdatesCorrectly() {
        ...
        // create a new issue and navigate to it
        app.buttons["New Issue"].tap()
        
        // find textfield and change issue name
        app.textFields["Enter the issue title here"].tap()
        app.textFields["Enter the issue title here"].clear()
        app.typeText("My New Issue")
        
        // go back in navigation and see if the name changed
        app.buttons["Issues"].tap()
