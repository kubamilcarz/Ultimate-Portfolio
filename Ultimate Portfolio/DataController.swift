...
class DataController: ObservableObject {
    ...
    /// Initializes a data controller, either in memoriy (for testing use such as previewing),
    /// or on permanent storage (For use in regualr app runs.) 
    ///
    ...
    /// any errors causes by saving, but this should be fine because
    ...
    func issuesForSelectedFilter() -> [Issue] {
        ...
        if filterEnabled {
            if filterPriority >= 0 {
                let prioriotyFilter = NSPredicate(format: "priority = %d", filterPriority)
                predicates.append(prioriotyFilter)
            }
    ...
    func hasEarned(award: Award) -> Bool {
        ...
        case "issues":
            // return true if they added a cartain number ofissues
...
//            fatalError("Unknonw award criterion: \(award.criterion)")
            return false
        }
    }
}
